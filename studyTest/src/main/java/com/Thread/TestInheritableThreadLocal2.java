...
public class TestInheritableThreadLocal2 {
    ...
    static BlockingQueue blockingQueue = new LinkedBlockingDeque(10);
    static ThreadPoolExecutor pool = new ThreadPoolExecutor(5, 5, 100, TimeUnit.MICROSECONDS, blockingQueue);
    ...
    private static class MyThreadTo extends Thread {
         ...
         ThreadPoolExecutor pool = new ThreadPoolExecutor(5, 5, 100, TimeUnit.MICROSECONDS, blockingQueue);

        @Override
        public void run() {
            pool.submit(()->{
                System.out.println("MyThreadTo inheritableThreadLocal = " + inheritableThreadLocal.get());
            });
