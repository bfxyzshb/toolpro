...
public class SlidingWindow {
    ...
    public boolean allow(int threshold) {  
        ...
        if (oldCursor != index) {
            ...
            // 清零，访问量不大时会有时间片跳跃的情况  
            clearBetween(oldCursor, index);
            System.out.println("clear:"+(index-oldCursor));
        }else{

        }
        for (int i = 1; i < windowSize; i++) {
            int tmp=(index - i + timeSliceSize) % timeSliceSize;
            sum += timeSlices[tmp].get();
        }
        // 阈值判断
        if (sum <= threshold) {  
            // 未超过阈值才+1  
    ...
    public static void main(String[] args) {
        SlidingWindow slidingWindow=new SlidingWindow(250,4);
        while(true){
            slidingWindow.allow(100);
            /*try {
                Thread.sleep(200);
